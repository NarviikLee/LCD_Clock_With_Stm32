...
이 프로젝트는 처음부터 완성된 구조를 설계한 뒤 구현한 결과물이 아니라, STM32를
학습하며 실제 부품을 하나씩 연결하고 발생한 문제를 직접 해결하는 과정에서 기능을
계속 확장해 완성한 프로젝트입니다.

시계 표시에서 시작해 DS1302 RTC, OLED 메뉴, DHT11 센서, 버튼, 알람과 타이머,
가변저항 입력, ESP32 통신까지 범위를 넓혔습니다. 이 과정에서 GPIO, I2C/UART 통신,
타이머와 PWM, 외부 인터럽트, 장치별 드라이버를 직접 경험할 수 있었습니다.

현재 코드는 당시의 시행착오와 문제 해결 과정을 보여주는 원본으로 보존하고 있습니다.
전역 상태 관리, 인터럽트와 메인 루프의 역할 구분, 센서 폴링 주기, 상태 머신 구조 등
지금 다시 설계한다면 더 명확하게 다듬을 부분도 있습니다. 하지만 동작하는 결과물을
직접 만들고 기능을 끝까지 통합했다는 점, 그리고 이후 더 나은 구조가 필요한 이유를
알게 되었다는 점에서 의미 있는 학습 기록입니다.

향후 리팩터링한다면 다음 순서로 개선할 계획입니다.

1. 기존 하드웨어 동작과 기능을 회귀 테스트로 먼저 고정
2. 인터럽트에서는 이벤트 플래그만 기록하고 실제 처리는 메인 루프로 이동
3. 전역변수를 기능별 상태 구조체로 통합
4. 메뉴, 알람, 타이머 동작을 명시적인 `enum` 상태 머신으로 정리
5. 드라이버 계층과 응용 로직을 분리하고 오류 및 입력값 검증 추가
